...
public class Scheduler {
	 ...
	 *
	 * @param strategy
	 *            The Strategy to negotiate thread management with.
	 */
	public Scheduler(AllocationStrategy strategy) {
		this.strategy = strategy;

		service = Executors.newScheduledThreadPool(strategy.getMaxThreads(),
				strategy::newThread);
	}
	 ...
	 *            The runnable to run.
	 * @param time
	 *            The time, in milliseconds from now, to run the task.
	 * @return A future to control the Runnable with
	 */
	 ...
	 *            The runnable to run.
	 * @param initialPeriod
	 *            The time, in milliseconds from now, to run the task first.
	 * @param interval
	 *            The time, in milliseconds from the previous interval, to run
	 *            the task after the initialPeriod.
	 * @return A future to control the Runnable with
	 */
	public ScheduledFuture<?> post(Runnable runnable, long initialPeriod,
			long interval) {
		return service.scheduleAtFixedRate(runnable, initialPeriod, interval,
				TimeUnit.MILLISECONDS);
	}

	/**
	 * Shuts down this Scheduler, stopping any further Runnables from being
	 * posted.
	 */
